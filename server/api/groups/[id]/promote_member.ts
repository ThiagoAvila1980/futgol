...
export default async function (req: any, res: any) {
    ...
    // UUID or Number support
    const match = url.match(/\/api\/groups\/([^\/]+)\/promote_member/);
    ...
    try {
        const groupRows = await sql(`SELECT admins FROM groups WHERE id = $1`, [groupId]) as any[];
        ...
        // Also update role in group_players for this specific group
        await sql(`UPDATE group_players SET role = 'admin' WHERE group_id = $1 AND player_id = $2`, [groupId, userId]);

        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ ok: true }));
    } catch (err: any) {
        console.error('Error in promote_member:', err);
        res.statusCode = 500;
